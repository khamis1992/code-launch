...
const BOLT_QUICK_ACTIONS_OPEN = '<codelaunch-quick-actions>';
const BOLT_QUICK_ACTIONS_CLOSE = '</codelaunch-quick-actions>';
    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          ...
          // Find all <codelaunch-quick-action ...>label</codelaunch-quick-action> inside
          const quickActionRegex = /<codelaunch-quick-action([^>]*)>([\s\S]*?)<\/codelaunch-quick-action>/g;
          let match;
          const buttons = [];
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    ...
    'data-codelaunch-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__boltQuickAction__\" data-codelaunch-quick-action=\"true\">${buttons.join('')}</div>`;
}
